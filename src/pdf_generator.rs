...
pub fn generate_pdf(
    ...
    let helvetica_font = doc
        .add_builtin_font(BuiltinFont::Helvetica)
        .map_err(|e| e.to_string())?; // Use map_err for error conversion
    let courier_font = doc
    ...
    let add_text =
        |layer: &PdfLayerReference, text: &str, x: Mm, y: f32, font: &IndirectFontRef| {
    ...
    add_text(
        &layer,
        &format!("Date: {}", invoice.date.format("%b %d, %Y")),
        Mm(15.0),
        y_pos,
        &helvetica_font,
    );
    ...
    add_text(
        &layer,
        &format!("Contact Phone: {}", invoice.customer.contact_phone),
        Mm(15.0),
        y_pos,
        &helvetica_font,
    );
    ...
    add_text(
        &layer,
        &format!("Due Date: {}", invoice.due_date.format("%b %d, %Y")),
        Mm(150.0),
        y_pos,
        &helvetica_font,
    );
    ...
    add_text(
        &layer,
        &format!("Balance Due: AU ${:.2}", invoice.total),
        Mm(150.0),
        y_pos,
        &helvetica_font,
    );
    ...
    add_text(
        &layer,
        &format!("(Current Date: {})", invoice.date.format("%b %d, %Y")),
        Mm(15.0),
        y_pos,
        &helvetica_font,
    );
    ...
    doc.save(&mut BufWriter::new(file))
        .map_err(|e| e.to_string())?; // Propagate save error
